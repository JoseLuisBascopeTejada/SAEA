...
2. **Detect (SCRFD-2.5GF, `det_2.5g.onnx`)**: bounding boxes + 5 landmarks per face, decoded from the model's 9 raw outputs (3 FPN strides × [score, bbox, keypoints]) via InsightFace's anchor-decode + `distance2kps` logic — verified empirically against our actual downloaded file (`use_kps=True`).
3. **Align**: affine transform to canonical 112×112 crop using those 5 points.
