...
- [x] **TSK-201** — Model acquisition script (via official `insightface` package)
  ...
  - **History**: briefly amended to also require `2d106det.onnx` for a separate landmark-extraction step, then reverted — empirical inspection of the actual downloaded `det_2.5g.onnx` (`get_outputs()`) showed it already includes a 5-point keypoint branch (9 outputs, `use_kps=True`), making a second landmark model unnecessary. `2d106det.onnx` may still be present in `backend/models_data/` from that earlier step but is unused.

- [x] **TSK-202** — Face detection module (bounding boxes + 5 landmarks)
  - Hat: AIBackendAgent
  - Depends on: TSK-201
