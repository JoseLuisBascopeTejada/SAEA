...
- **RF-02 Detection & alignment**: Backend detects faces using SCRFD-2.5GF (`det_2.5g.onnx`, from the official InsightFace `buffalo_m` pack) and aligns each detected face to a 112×112 px crop using the 5 facial landmarks the model outputs directly (affine transform).

> **Landmark source (verified empirically during TSK-202, corrected from an earlier assumption)**: an initial research pass, based on InsightFace's naming convention (`_kps` suffix implies keypoints), assumed `det_2.5g.onnx` had no landmark output and planned to use a separate `2d106det.onnx` model (106 points) instead. Running `onnxruntime.InferenceSession(...).get_outputs()` directly against our actual downloaded file showed 9 outputs (3 scores + 3 bboxes + 3×10-dim keypoints per FPN stride level, i.e. `use_kps=True`) — this specific `buffalo_m` build DOES include a keypoint branch. The design reverts to the original single-model approach: `det_2.5g.onnx` provides bounding boxes and 5 landmarks together, decoded via InsightFace's `distance2kps` logic. `2d106det.onnx` is not used in the pipeline (the file may remain in `backend/models_data/` from the earlier download but is not loaded by any service). Lesson: naming-convention-based assumptions about ONNX model outputs must always be confirmed empirically against the actual file before writing decode logic.
