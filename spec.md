...
- **RF-02 Detection & alignment**: Backend detects faces using SCRFD-2.5GF (`det_2.5g.onnx`, from the official InsightFace `buffalo_m` pack) to get bounding boxes. `det_2.5g.onnx` does not output facial landmarks (confirmed empirically during TSK-202 — 6 output tensors, no keypoint branch). A second model, `2d106det.onnx` (also from `buffalo_m`, 106-point 2D landmarks), is run per detected face to extract the 5 standard points (left eye, right eye, nose tip, left mouth corner, right mouth corner) by index-mapping from its 106-point output. Each face is then aligned to a 112×112 px crop using those 5 points (affine transform).

> **Landmark model decision (added after TSK-202 research)**: `det_2.5g.onnx` in the `buffalo_m` pack has no keypoint output — landmarks come from the separate `2d106det.onnx` model already included in the same pack. This requires copying `2d106det.onnx` into `backend/models_data/` (a retroactive amendment to TSK-201) and adds `MODEL_LANDMARK_PATH` to `.env.example`. Task TSK-202B covers implementing the 106→5 point extraction; TSK-203 (alignment) depends on it instead of on TSK-202 alone.
