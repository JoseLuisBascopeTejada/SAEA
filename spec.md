...
- **RF-02 Detection & alignment**: Backend detects faces using SCRFD-2.5GF (ONNX) and aligns each detected face to a 112×112 px crop using the 5 facial landmarks (affine transform).
- **RF-03 Embedding & matching**: Generate a 512-d embedding per aligned face using ArcFace MobileFaceNet (INT8, ONNX). Run cosine similarity search against `pgvector` with threshold ≥ 0.42. Highest score above threshold wins; ties go to "unrecognized".
